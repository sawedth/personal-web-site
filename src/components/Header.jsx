    ...
    return (
        <div className={"header h-[671px] px-[16%]" + (mode ? "" : " koyu")}>
            <div className="preferances flex  justify-end  pt-[1rem]">
                    {language && <span className={( mode ? "text-[#a69ffffc] " : "text-[#777777] " ) + ' mr-[45px] cursor-pointer'} onClick={() => changeLanguage()}><span className={(mode ? 'text-[#CBF281]' : 'text-[#8F88FF]')+ ""}>TÜRKÇE</span>'YE GEÇ </span>}
                    {!language && <span className={( mode ? "text-[#a69ffffc] " : "text-[#777777] ") + ' mr-[45px] cursor-pointer'} onClick={() => changeLanguage()}>{translate}<span className={(mode ? 'text-[#CBF281]' : 'text-[#8F88FF]')+ ""}>{languageName}</span></span>}
                ...
                <span className={(mode ? "text-[#4731D3] " : "text-[#D9D9D9] ") +' ml-[3px] text-[15px] font-bold tracking-wide'}>{mode ? "DARK MODE" : "LIGHT MODE"}</span>
            </div>
            ...
            <div className='personal-info h-[50%] flex flex-row justify-between mt-[2rem] '>
                <div className='personal-exp pt-[4rem] text-left flex flex-col w-[60%] justify-between m-[0px]'>
                    <h1 className='text-[#CBF281] text-[54px] text-base/14 font-bold'>{language ? "I am a Frontend Developer..." : head}</h1>
                    <p className='text-[white] text-[24px]'> {language ? "I...who likes to craft solid and scalable frontend products with great user experiences." : body}</p>
